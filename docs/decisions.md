...
## R2-3 — Profiles are private by default; a connection never grants trip access (Phase 20 / B3) ✅ [LOCKED]
The identity layer separates three things that are easy to conflate: **profile visibility**, the **friend graph**, and **trip membership**. (1) A profile is **private by default** (`visibility` on `profiles`, existing rows backfilled private); the rewritten `profiles_select` policy makes a row readable only by the owner, a **co-trip-member** (trip function must keep working), or — when **not blocked** either way — a **public** profile or an **accepted connection**. The B2 `passport_stats` read is widened to the *same* rule so a viewable profile can show the outward passport snapshot. (2) The `connections` graph (request/accept/**block**, one row per unordered pair) is mutated **only through SECURITY DEFINER RPCs** — the table has no direct write policy, so there is no illegal-write surface — and a **block hides both users on every world surface, both directions**. (3) **The hard rule:** being a connection, or a profile being public, **never** grants access to another user's trip content — chat/money/media/journal/member list stay gated on `trip_members`, exactly as before. Provenance (`get_profile_provenance`) only ever returns trips the **viewer is also a member of**, and mutuals are a **count only**. This is stated as code in `lib/social.ts` (`connectionGrantsTripAccess` is constant-false; `profileVisibleTo` mirrors the SQL) and **pinned by `__tests__/social.test.tsx`** — a fully-visible profile still cannot render trip content. **Deferred to Phase 21 (Safety):** the *stronger* block guarantee that also severs a block pair inside a shared trip, plus reports + age-gate; the seams are noted in the migration. Only recap + passport remain the outward-shareable artifacts (unchanged).

## R2-4 — Safety ships before discovery: age band (not DOB), one-tap reports, block-everywhere, moderator queue (Phase 21 / B4) ✅ [LOCKED]
Trust-and-safety is built **before** Nearby/discovery (Phase 22), never after — Nearby literally cannot honour its own "under-18 excluded both directions" rule without an age band, so Safety comes first. Decisions locked here: (1) **Store an age _band_, never the birthdate.** `set_age_band(dob)` derives `adult|minor` and persists **only the band** (privacy-minimal); a **DB trigger forces a minor's profile private** and both `profiles_select` (public branch) and `search_profiles` exclude minors, suspended, and no-age users — so under-18 is *never* public or discoverable, enforced in three places (trigger + policy + client `canBePublic`/`isDiscoverable`), and pinned by `__tests__/safety.test.tsx`. (2) **Reports are one tap from every user-generated surface**, with a fixed subject×reason taxonomy, **insert-your-own + moderator-read**, and rate-limited — a reusable `<ReportSheet>`/`<ReportAction>` wired on profiles + chat (other surfaces adopt it in one line). (3) **Block is bidirectional and now reaches chat** (`messages_select` drops a blocked pair's messages, per-subscriber over Realtime), completing the "stronger block guarantee" the B3 note deferred. (4) **Rate limits** on connection requests (30/hr) and messages (20/60s), both also blocked for suspended accounts. (5) **A minimal moderator tool** (`app/moderation.tsx`, gated to a `profiles.is_moderator` flag, surfaced in Settings only for staff): dismiss / remove content / **suspend**, each written to a `moderation_actions` audit trail; a suspended account drops out of discovery and can't write. **Deliberately deferred:** real moderator *provisioning* (the flag is flipped in the DB by an operator for now — no self-serve admin), an in-app appeals flow, and DM-specific limits (no 1:1 DM feature exists yet — the message limit covers the only messaging surface). Safety governs *world* surfaces + the age/block/report machinery only; **trip content stays gated on membership** (unchanged).

## R2-5 — Nearby uses the trip's public destination, never device location; opt-in, mutual, safety-gated (Phase 22 / B5) ✅ [LOCKED]
Nearby Travelers is built **after** Safety (B4), and its hardest question — location privacy (the open §12-5) — is answered by **construction**: the matching "area" is a **geohash of the trip's PUBLIC destination** (`trips.location_lat/lng`), so the app **never collects, stores, or shows a user's live/precise location**. The geohash is stored at precision 5 (still only the public destination) and matched on the **precision-3 prefix (~150 km region)** — coarse by design, forgiving of city boundaries, and impossible to reverse into a precise point. Other locked choices: **off by default** (a `discovery_optins` row exists only after an explicit opt-in); **mutual visibility** (`find_nearby_travelers` returns nobody unless the caller is opted in) with **mutual-consent contact** via the existing B3 connection request (Nearby reveals only a minimal public identity, never a way to message directly); **adult-only** (`set_nearby_optin` requires a verified adult + trip member + not suspended) with **minors, suspended, and blocked users excluded both directions** in matching; **auto-expiry** (matches filter to windows that haven't ended; stale rows are harmless); and **immediate opt-out** (disabling deletes the row). **Deliberately deferred:** live-location / "who's physically near me right now" (would require device GPS + a much heavier consent model — explicitly out), neighbor-cell matching across geohash boundaries (single-region prefix is the coarse MVP), and any background location. The area geohash is derived client-side from the public destination and the window is read server-side from the trip; a fudged area only ever surfaces other opted-in adults who consented to discovery, and contact still needs mutual consent — so the privacy surface stays minimal. Proven by `__tests__/nearby.test.tsx`.

## R2-6 — Music on shares is a swappable, cleared-catalogue provider that ships not-configured; export-without-music is equally prominent (Phase 23 / B6) ✅ [LOCKED]
Music is **optional, licensed, and honest**. Locked choices: (1) The catalogue is a **swappable `MusicCatalogProvider`** the operator plugs in (`EXPO_PUBLIC_MUSIC_PROVIDER`); the app **ships with none configured** (`getMusicProvider()` → null) and the picker shows a clear **"not set up yet"** state. **Cleared/licensed catalogues only — no Spotify/Apple/commercial-audio, and no user uploads.** A built-in **example** catalogue (metadata only, no audio) exists solely to exercise the UI. (2) The picker is the spec's **Instagram-style pick / preview / trim** — a searchable list, a **draggable 15-second window**, and `expo-audio` **preview** (mounted only when a track has a cleared preview URL). (3) The choice + trim + **retained rights metadata** persist to `share_audio` (one per trip, member RLS). (4) **Export without music is equally prominent** — the recap's "Share / download recap" stays the primary path and always works; music is an explicit optional "Add music." **Deliberately deferred (documented seam):** actually **baking audio into the exported file**. The current recap export is **image-only**; muxing audio needs the **Phase-2 video recap** (D9, which parks the auto video montage). Rather than fake it, the picker says so ("music plays on video recaps"), and the stored selection is ready for that phase. Adding `expo-audio` (a native module) now is deliberate infrastructure for preview + the coming video export. Proven by `__tests__/music.test.tsx` (ships not-configured; trim clamps to a 15s window). **This completes Release 2 (Phases 18–23 / design-system A1–A3 + B1–B6).**
